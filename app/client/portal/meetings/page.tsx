...
const tk = {
  bg:           "#f8f8f8",
  surface:      "#ffffff",
  surfaceRaised:"#f1f2ec",
  border:       "rgba(8,8,8,0.10)",
  borderStrong: "rgba(8,8,8,0.20)",
  ...
  green:      "#237d63",
  greenLight: "rgba(35,125,99,0.12)",
  greenMid:   "#2e9b7b",
  greenText:  "#237d63",
...
.cp-btn-danger {
  ...
  border: 1px solid rgba(185,67,62,0.16);
  padding: 0 14px; height: 36px;
}
.cp-btn-danger:hover:not(:disabled) {
  background: rgba(185,67,62,0.16); border-color: rgba(185,67,62,0.28);
}
.cp-btn-ghost {
...
.cp-choice.sel {
  ...
  box-shadow: 0 4px 14px rgba(185,67,62,0.14); transform: translateY(-1px);
}
.cp-choice-ico {
...
.cp-choice.sel .cp-choice-ico { background: rgba(185,67,62,0.12); color: ${tk.red}; }
...
.cp-toggle.sel { border-color: ${tk.amberText}; background: ${tk.accentLight}; color: ${tk.accentText}; box-shadow: 0 2px 8px rgba(255,158,27,0.18); }
    ...
    <Modal wide title="Fiche du rendez-vous" subtitle={`${cn_name} · ${companyName}`} onClose={onClose}
      ...
      <Sec>
        ...
        {m.cancellationReason && (
          <div style={{padding:"10px 14px",background:tk.redLight,border:"1px solid rgba(185,67,62,0.16)",borderRadius:10,fontSize:12.5,color:tk.redText,fontStyle:"italic"}}>
    ...
    <Modal title="Retour enregistré" onClose={onClose}
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"56px 32px",textAlign:"center"}}>
        ...
        <p style={{fontSize:13.5,color:tk.ink3,maxWidth:280,lineHeight:1.6,margin:0}}>
          Votre avis a été transmis à votre équipe Prospecto.
        </p>
      </div>
    </Modal>
  );
    ...
    <Modal title="Feedback sur le rendez-vous" subtitle={`${name} · ${companyName}`} onClose={onClose}
      ...
      <Sec label="Commentaire" last>
        <textarea className="cp-textarea" value={note} onChange={e=>onNote(e.target.value)} rows={4}
          ...
          style={{minHeight:100}} />
        <p style={{fontSize:11,color:tk.ink4,marginTop:8}}>Visible uniquement par votre équipe Prospecto.</p>
      </Sec>
    </Modal>
  );
}
...
function RsModal({ m, onClose, date, time, sub, onDate, onTime, onSubmit }: {
  ...
  return (
    <Modal title="Demander un report" subtitle={`${name} · ${companyName}`} onClose={onClose}
      ...
      <Sec>
        <div style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px",background:tk.surfaceRaised,borderRadius:12,border:"1px solid rgba(12,59,56,0.12)"}}>
